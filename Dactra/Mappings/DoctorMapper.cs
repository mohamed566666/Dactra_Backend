    ...
    public class DoctorMapper : Profile
    {
        public DoctorMapper()
        {
            ...
            CreateMap<DoctorProfile, DoctorProfileResponseDTO>()
                ...
                .ForMember(dest => dest.Address,opt => opt.MapFrom(src => src.Address != null ? src.Address : string.Empty))
                .ForMember(dest => dest.age,opt => opt.MapFrom(src =>src.DateOfBirth != default(DateTime) ? CalculateYears(src.DateOfBirth) : 0))
                .ForMember(dest => dest.YearsOfExperience,opt => opt.MapFrom(src =>src.StartingCareerDate != default(DateTime) ? CalculateYears(src.StartingCareerDate) : 0))
                .ForMember(dest => dest.AverageRating,opt => opt.MapFrom(src => src.Avg_Rating == default(decimal) ? 0m : src.Avg_Rating))
            ...
            CreateMap<DoctorProfile, DoctorsResponseDTO>()
                ...
                .ForMember(dest => dest.SpecializationName,opt => opt.MapFrom(src => src.specialization != null ? src.specialization.Name : string.Empty))
                .ForMember(dest => dest.age,opt => opt.MapFrom(src =>src.DateOfBirth != default? CalculateYears(src.DateOfBirth): 0))
                .ForMember(dest => dest.YearsOfExperience,opt => opt.MapFrom(src =>src.StartingCareerDate != default? CalculateYears(src.StartingCareerDate): 0))
                .ForMember(dest => dest.AverageRating,opt => opt.MapFrom(src => src.Avg_Rating))
                .ForMember(dest => dest.About,opt => opt.MapFrom(src => src.About ?? string.Empty))
